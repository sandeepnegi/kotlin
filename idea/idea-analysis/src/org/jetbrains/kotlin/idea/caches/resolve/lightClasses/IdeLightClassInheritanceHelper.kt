    ...
    private fun isAmongEntries(baseClass: PsiClass, entries: List<KtSuperTypeListEntry>): ImpreciseResolveResult {
        ...
        entries@ for (entry in entries) {
            val reference: KtSimpleNameExpression =
                    when (entry) {
                        is KtSuperTypeCallEntry -> entry.calleeExpression.constructorReferenceExpression
                        is KtDelegatedSuperTypeEntry -> entry.typeAsUserType?.referenceExpression
                        is KtSuperTypeEntry -> entry.typeAsUserType?.referenceExpression
                        else -> error("Unexpected supertype list entry of type ${entry.javaClass}")
                    } ?: continue@entries
